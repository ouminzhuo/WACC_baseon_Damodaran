...
- `apply_vat`, `apply_wht`, `apply_fx_hedge`, `enforce_country_wht`, `allow_stale_wht`, `max_wht_age_days`, `enforce_tax_from_tables`, `error_on_tax_input_mismatch`
- `usd_10y_bond_rate`
...
```bash
python americas-wacc/scripts/refresh_wht_table.py --out WHT_ctry.csv --set "Canada=0.25|https://taxsummaries.pwc.com/canada/corporate/withholding-taxes"
...
税项判定优先以 VAT_ctry/WHT_ctry 为准；输入开关仅用于一致性校验。
